...
async function createfriend(requestee_id, requester_id) {
  ...
  var friend = await db.query(
    "INSERT INTO friends (requestee_id,requester_id,requested_on) VALUES ($1,$2,$3) RETURNING *;",
...
async function getfriend(userid) {
  var friend = await db.query(
    "SELECT requestee_id, requester_id,requested_on,Approved_on,Declined_on  FROM friends INNER JOIN users ON users.id=friends.requestee_id OR users.id=friends.requester_id WHERE users.id=$1 ;",
    [userid],
    (err, res) => {
      if (err) {
        console.log(err);

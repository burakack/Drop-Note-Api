...
{
    var note=await db.query('SELECT nickname,title,notetext,is_anonymus,likecount,dislikecount,notes.created_at,notes.updated_at,notes.deleted_at FROM notes INNER JOIN users ON notes.userid=users.id WHERE title=$1'
    ,[title]
    ,(err,res)=>{
        if(err){
            console.log(err);
